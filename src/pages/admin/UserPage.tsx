...
import { listUsers, softDeleteUser, hardDeleteUser, type Page, type ResUser } from "../../services/admin/users";
            ...
            {!loading && data && (
                <>
                                ...
                                data.content.map((u) => (
                                    <tr key={u.id} className="border-t hover:bg-gray-50/60">
                                        ...
                                        <td className="px-4 py-3 space-x-2">
                                            <button className="px-3 py-1 rounded-md bg-amber-50 text-amber-700 hover:bg-amber-100" onClick={() => onSoft(u.id)}>Soft</button>
                                            <button className="px-3 py-1 rounded-md bg-rose-50 text-rose-700 hover:bg-rose-100" onClick={() => onHard(u.id)}>Hard</button>
                                        </td>
                                    </tr>
                                ))
                            )}
                    ...
                    <div className="flex items-center gap-2 justify-end mt-4">
                        <button
                            className="px-3 py-1 border rounded rounded-lg bg-gray-100 hover:bg-gray-200 disabled:opacity-50"
