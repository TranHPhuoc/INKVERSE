...
function normalizeCart(payload: any): CartItem[] {
    ...
    return raw.map((x: any, idx: number): CartItem => {
        ...
        return {
            ...
            thumbnail: x.thumbnail ?? b.thumbnail ?? null,
            price: Number(x.effectivePrice ?? x.price ?? b.effectivePrice ?? b.price ?? 0),
        ...
        <div className="min-h-screen bg-gray-50">
            <div className="mx-auto max-w-7xl px-4 md:px-6 py-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_420px] gap-6">
                    {/* LEFT: Product list (bigger) */}
                    <section className="bg-white border rounded-2xl shadow-sm p-5">
                        <h2 className="text-lg font-semibold mb-4">Sản phẩm</h2>

                        {loadingCart ? (
                            <div className="py-14 text-center text-gray-500 text-base">Đang tải giỏ hàng…</div>
                        ) : items.length === 0 ? (
                            <div className="py-14 text-center text-gray-500 text-base">Không có sản phẩm nào để thanh toán.</div>
                        ) : (
                            <>
                                    ...
                                    {items.map((it) => (
                                        <div key={it.id} className="flex items-start gap-5 py-4">
                                            ...
                                            <div className="text-right">
                                                ...
                                                <div className="text-xs md:text-sm text-gray-500">{formatVND(it.price)} ₫ / sp</div>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                    ...
                    {/* RIGHT: Checkout form (bigger text) */}
                    <aside className="space-y-4">
                        <section className="bg-white border rounded-2xl shadow-sm p-5 space-y-5 text-[15px] leading-6">
                            ...
                            <button
                                ...
                                className="w-full h-12 rounded-lg bg-rose-600 text-white text-base font-semibold hover:opacity-90 disabled:opacity-60"
                            >
