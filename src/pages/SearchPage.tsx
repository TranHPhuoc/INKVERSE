        ...
        (async () => {
            ...
            try {
                const data = await searchBooks({
                    ...
                    authorId, categoryId, publisherId, supplierId,
                });
            ...
            <main className="flex-1 bg-gray-50">
                <div className="mx-auto max-w-7xl px-4 md:px-6 py-6">
                    ...
                    <div className="mb-4 flex flex-wrap gap-2 text-xs">
                        <Link to={`/search?${new URLSearchParams({ q, sort: "createdAt", direction: "DESC" }).toString()}`}
                              className={`px-3 py-1.5 rounded border ${sort==="createdAt" ? "bg-gray-900 text-white" : "bg-white"}`}>
                            Mới nhất
                        </Link>
                        <Link to={`/search?${new URLSearchParams({ q, sort: "price", direction: "ASC" }).toString()}`}
                              className={`px-3 py-1.5 rounded border ${sort==="price" && direction==="ASC" ? "bg-gray-900 text-white" : "bg-white"}`}>
                            Giá tăng dần
                        </Link>
                        <Link to={`/search?${new URLSearchParams({ q, sort: "price", direction: "DESC" }).toString()}`}
                              className={`px-3 py-1.5 rounded border ${sort==="price" && direction==="DESC" ? "bg-gray-900 text-white" : "bg-white"}`}>
                            Giá giảm dần
                        </Link>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        {loading
                            ? Array.from({ length: 10 }).map((_, i) => (
                                <div key={i} className="rounded-xl border bg-white p-4 animate-pulse h-[300px]" />
                            ))
