...
import { LogOut, User as UserIcon, Home, ShoppingCart } from "lucide-react";
...
export default function SaleTopbar() {
  ...
  const userLabel = useMemo(() => user?.name || user?.email || "Sale", [user?.name, user?.email]);
      ...
      <div className="flex items-center gap-2 px-4">
        ...
        <div className="relative" onMouseEnter={openNow} onMouseLeave={closeSoon}>
          ...
          >
            ...
            <Link
              to="/"
              className="flex w-full cursor-pointer items-center gap-2 px-4 py-2 text-left text-sm hover:bg-gray-50"
              role="menuitem"
