...
export default function BookGallery({ images, className, onIndexChange }: Props) {
    ...
    if (!sorted.length) {
        return (
            <div className={className}>
                <div className="w-full aspect-[3/4] rounded-xl bg-gray-100" />
            </div>
        );
    }
    ...
    return (
        <div className={className}>
            {/* Main image */}
            <div
                className="relative rounded-xl border bg-gray-50 overflow-hidden"
                tabIndex={0}
                onKeyDown={(e) => {
            ...
            >
                <AnimatePresence mode="wait">
                    <motion.img
                        ...
                        alt="book image"
                        className="main-image w-full h-auto aspect-[3/4] object-cover block rounded-xl"
                ...
                {sorted.length > 1 && (
                    <>
                        <button
                            ...
                            onClick={() => go(-1)}
                            className="absolute left-2 top-1/2 -translate-y-1/2 rounded-full bg-white/70 hover:bg-white shadow px-2 py-2"
                        >
                            ‹
                        </button>
                        <button
                            aria-label="Ảnh sau"
                            onClick={() => go(1)}
                            className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-white/70 hover:bg-white shadow px-2 py-2"
                        >
                            ›
                        </button>
                    </>
            ...
            {sorted.length > 1 && (
                <div className="mt-3 flex items-center gap-2">
                    ...
                    >
                        {sorted.map((img, i) => {
                            const activeCls =
                                i === active ? "ring-2 ring-rose-500" : "hover:shadow";
                            return (
                                <button
                                    ...
                                    onClick={() => setActive(i)}
                                    aria-label={`Ảnh ${i + 1}`}
                                    className={`relative w-20 h-28 rounded border overflow-hidden flex-shrink-0 ${activeCls}`}
                                >
                                    <img
                                        src={pickThumb(img)}
                                        alt=""
